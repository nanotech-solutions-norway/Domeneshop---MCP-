"""Validate the Phase 40 operator approval gate."""
...
REPORTS = {
    ...
    39: "phase39-write-preflight-validation-report.json",
    40: "phase40-operator-approval-validation-report.json",
}
ENV_DEFAULTS = {
...
def main() -> int:
    parser = argparse.ArgumentParser(description="Validate the Phase 40 operator approval gate.")
    parser.add_argument("--repo-root", default=".")
    parser.add_argument("--output", default="phase40-operator-approval-validation-report.json")
    ...
    payload = {
        "summary": {
            "mode": "phase40_operator_approval_validation",
            "phase40_operator_approval": "OPERATOR_APPROVAL_GATE_ONLY" if passed else "CHECK_FAILED",
            "release_decision": "HOLD_PHASE40_OPERATOR_APPROVAL_GATE_ONLY" if passed else "HOLD_FOR_FIX",
            "passed": passed,
...
def validate(root: Path) -> list[Check]:
    ...
    phase_doc = read(root / "docs" / DOCS[40])
    combined = "\n".join(read(root / "docs" / name) for name in DOCS.values()) + "\n" + read(root / "README.md")
    checks.append(Check("hold:phase40", "HOLD_PHASE40_OPERATOR_APPROVAL_GATE_ONLY" in combined, "Required Phase 40 hold marker."))
    checks.append(Check("hold:live", "HOLD_LIVE_CHANGE_ACTIVATION" in combined, "Required live hold marker."))
    for marker in REQUIRED_MARKERS:
        checks.append(Check(f"approval:{marker}", marker in phase_doc, f"Required marker: {marker}."))
    checks.append(Check("sequence:remaining", "Phase 41" in combined and "Phase 42" in combined, "Required remaining sequence markers."))
