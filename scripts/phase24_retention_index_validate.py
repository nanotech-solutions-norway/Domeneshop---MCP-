"""Validate the Phase 24 retention index gate."""
...
PHASE_DOCS = [
    ...
    Path("docs/PHASE19_RELEASE_FREEZE_GATE.md"),
    Path("docs/PHASE20_HANDOFF_PACKAGE_GATE.md"),
    Path("docs/PHASE21_REVIEW_CLOSURE_GATE.md"),
    Path("docs/PHASE22_MAINTENANCE_BASELINE_GATE.md"),
    Path("docs/PHASE23_ARCHIVE_INDEX_GATE.md"),
    Path("docs/PHASE24_RETENTION_INDEX_GATE.md"),
]
PHASE_VALIDATORS = [
    ...
    Path("scripts/phase19_release_freeze_validate.py"),
    Path("scripts/phase20_handoff_package_validate.py"),
    Path("scripts/phase21_review_closure_validate.py"),
    Path("scripts/phase22_maintenance_baseline_validate.py"),
    Path("scripts/phase23_archive_index_validate.py"),
    Path("scripts/phase24_retention_index_validate.py"),
]
...
REQUIRED_REPORT_NAMES = (
    ...
    "phase19-release-freeze-validation-report.json",
    "phase20-handoff-package-validation-report.json",
    "phase21-review-closure-validation-report.json",
    "phase22-maintenance-baseline-validation-report.json",
    "phase23-archive-index-validation-report.json",
    "phase24-retention-index-validation-report.json",
...
def main() -> int:
    parser = argparse.ArgumentParser(description="Validate the Phase 24 retention index gate.")
    parser.add_argument("--repo-root", default=".", help="Repository root. Defaults to current directory.")
    parser.add_argument("--output", default="phase24-retention-index-validation-report.json", help="JSON report path.")
    args = parser.parse_args()

    root = Path(args.repo_root).resolve()
    checks = validate_phase24_retention_index(root)
    passed = all(check.passed for check in checks)

    payload = {
        "summary": {
            "mode": "phase24_retention_index_validation",
            "phase24_retention": "RETENTION_INDEX_ONLY" if passed else "CHECK_FAILED",
            "release_decision": "HOLD_PHASE24_RETENTION_INDEX_ONLY" if passed else "HOLD_FOR_FIX",
            "passed": passed,
...
def validate_phase24_retention_index(repo_root: Path) -> list[Check]:
    checks: list[Check] = []
    ...
    for marker in REQUIRED_HOLD_MARKERS:
        checks.append(Check(f"hold_marker:{marker}", marker in combined_text, f"Required marker must appear in retention index text: {marker}."))
