"""Validate the Phase 35 release closure."""
...
def main() -> int:
    parser = argparse.ArgumentParser(description="Validate the Phase 35 release closure.")
    parser.add_argument("--repo-root", default=".")
    parser.add_argument("--output", default="phase35-release-closure-validation-report.json")
    ...
    payload = {
        "summary": {
            "mode": "phase35_release_closure_validation",
            "phase35_release_closure": "RELEASE_CLOSURE_ONLY" if passed else "CHECK_FAILED",
            "release_decision": "HOLD_PHASE35_RELEASE_CLOSURE_ONLY" if passed else "HOLD_FOR_FIX",
            "passed": passed,
...
def validate(root: Path) -> list[Check]:
    ...
    checks.append(Check("hold:phase35", "HOLD_PHASE35_RELEASE_CLOSURE_ONLY" in combined, "Required Phase 35 hold marker."))
    checks.append(Check("hold:live", "HOLD_LIVE_CHANGE_ACTIVATION" in combined, "Required live hold marker."))
    checks.append(Check("sequence:write-readiness", "Phase 36" in combined and "Phase 42" in combined, "Required write-readiness sequence markers."))
