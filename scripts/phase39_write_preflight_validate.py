"""Validate the Phase 39 write preflight and dry-run gate."""
...
def main() -> int:
    parser = argparse.ArgumentParser(description="Validate the Phase 39 write preflight and dry-run gate.")
    parser.add_argument("--repo-root", default=".")
    parser.add_argument("--output", default="phase39-write-preflight-validation-report.json")
    ...
    payload = {
        "summary": {
            "mode": "phase39_write_preflight_validation",
            "phase39_write_preflight": "WRITE_PREFLIGHT_DRY_RUN_ONLY" if passed else "CHECK_FAILED",
            "release_decision": "HOLD_PHASE39_WRITE_PREFLIGHT_DRY_RUN_ONLY" if passed else "HOLD_FOR_FIX",
            "passed": passed,
...
def validate(root: Path) -> list[Check]:
    ...
    phase_doc = read(root / "docs" / DOCS[39])
    combined = "\n".join(read(root / "docs" / name) for name in DOCS.values()) + "\n" + read(root / "README.md")
    checks.append(Check("hold:phase39", "HOLD_PHASE39_WRITE_PREFLIGHT_DRY_RUN_ONLY" in combined, "Required Phase 39 hold marker."))
    checks.append(Check("hold:live", "HOLD_LIVE_CHANGE_ACTIVATION" in combined, "Required live hold marker."))
    for marker in REQUIRED_MARKERS:
        checks.append(Check(f"preflight:{marker}", marker in phase_doc, f"Required marker: {marker}."))
    checks.append(Check("sequence:remaining", "Phase 40" in combined and "Phase 42" in combined, "Required remaining sequence markers."))
