"""Validate the Phase 37 credential readiness gate."""
...
def main() -> int:
    parser = argparse.ArgumentParser(description="Validate the Phase 37 credential readiness gate.")
    parser.add_argument("--repo-root", default=".")
    parser.add_argument("--output", default="phase37-credential-readiness-validation-report.json")
    ...
    payload = {
        "summary": {
            "mode": "phase37_credential_readiness_validation",
            "phase37_credential_readiness": "CREDENTIAL_READINESS_ONLY" if passed else "CHECK_FAILED",
            "release_decision": "HOLD_PHASE37_CREDENTIAL_READINESS_ONLY" if passed else "HOLD_FOR_FIX",
            "passed": passed,
...
def validate(root: Path) -> list[Check]:
    ...
    phase_doc = read(root / "docs" / DOCS[37])
    combined = "\n".join(read(root / "docs" / name) for name in DOCS.values()) + "\n" + read(root / "README.md")
    checks.append(Check("hold:phase37", "HOLD_PHASE37_CREDENTIAL_READINESS_ONLY" in combined, "Required Phase 37 hold marker."))
    checks.append(Check("hold:live", "HOLD_LIVE_CHANGE_ACTIVATION" in combined, "Required live hold marker."))
    for marker in REQUIRED_MARKERS:
        checks.append(Check(f"credential:{marker}", marker in phase_doc, f"Required marker: {marker}."))
    checks.append(Check("sequence:remaining", "Phase 38" in combined and "Phase 42" in combined, "Required remaining sequence markers."))
