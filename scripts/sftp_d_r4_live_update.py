"""Exact one-file live UPDATE gate for D-R4 SFTP validation.

Authorization scope is intentionally hard-coded. This script updates only the
known validation target from one exact before hash to one exact after payload.
It does not expose a reusable SFTP write client and cannot delete or rename.
"""
...
RELEASE_ID = "D-R4-SFTP-UPDATE-20260824-001"
TARGET = "/www/.mcp-d-r4-validation.txt"
PAYLOAD = b"mcp-validation=D-R4-SFTP-UPDATE-20260824\n"
TARGET_SHA256 = "0619d5e786da50c431b090667936a2076b7bbd1054bda5758c30d74eec7f2f2a"
BEFORE_SHA256 = "9e37287a5e29a0d911589ac470ce83c5cabfeba6f0672c24935b6c2df96aa21a"
AFTER_SHA256 = "482c668063d3b849337b82d5c04dcbcb7fb65a8ccfdae3226a61c5c7e4527203"
...
def _write_exact(config: SftpReadConfig) -> None:
    ...
    try:
        ...
        # This is the one explicitly authorized overwrite of the exact target.
...
def main() -> int:
    ...
    try:
        ...
        if os.environ.get("SFTP_D_R4_UPDATE_AUTHORIZED", "false").strip().lower() != "true":
            raise RuntimeError("exact_sftp_update_authorization_missing")
        ...
        if before.get("sha256") != BEFORE_SHA256:
            _emit(success=False, status="before_state_mismatch_hold", provider_mutation_performed=False,
                  sftp_update_authorized=True, independent_readback_verified=False,
        ...
        if not verified:
            _emit(success=False, status="updated_but_readback_failed_hold", provider_mutation_performed=True,
                  sftp_update_authorized=True, independent_readback_verified=False,
                  automatic_delete_performed=False)
            return 3

        _emit(success=True, status="updated_and_readback_verified", provider_mutation_performed=True,
              sftp_update_authorized=True, independent_readback_verified=True,
              automatic_delete_performed=False)
        return 0
    except Exception as exc:
        _emit(success=False,
              status="updated_but_exception_hold" if mutated else "pre_update_hold",
              bounded_error_class=exc.__class__.__name__,
              provider_mutation_performed=mutated,
              sftp_update_authorized=True,
