"""Validate the Phase 23 archive index gate."""
...
def main() -> int:
    parser = argparse.ArgumentParser(description="Validate the Phase 23 archive index gate.")
    parser.add_argument("--repo-root", default=".", help="Repository root. Defaults to current directory.")
    parser.add_argument("--output", default="phase23-archive-index-validation-report.json", help="JSON report path.")
    args = parser.parse_args()

    root = Path(args.repo_root).resolve()
    checks = validate_phase23_archive_index(root)
    passed = all(check.passed for check in checks)

    payload = {
        "summary": {
            "mode": "phase23_archive_index_validation",
            "phase23_archive": "ARCHIVE_INDEX_ONLY" if passed else "CHECK_FAILED",
            "release_decision": "HOLD_PHASE23_ARCHIVE_INDEX_ONLY" if passed else "HOLD_FOR_FIX",
            "passed": passed,
...
def validate_phase23_archive_index(repo_root: Path) -> list[Check]:
    checks: list[Check] = []
    ...
    for marker in REQUIRED_HOLD_MARKERS:
        checks.append(Check(f"hold_marker:{marker}", marker in combined_text, f"Required marker must appear in archive index text: {marker}."))
