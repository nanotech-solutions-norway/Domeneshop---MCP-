"""Validate the Phase 33 checkpoint."""
...
def main() -> int:
    parser = argparse.ArgumentParser(description="Validate the Phase 33 checkpoint.")
    parser.add_argument("--repo-root", default=".")
    parser.add_argument("--output", default="phase33-checkpoint-validation-report.json")
    ...
    payload = {
        "summary": {
            "mode": "phase33_checkpoint_validation",
            "phase33_checkpoint": "CHECKPOINT_ONLY" if passed else "CHECK_FAILED",
            "release_decision": "HOLD_PHASE33_CHECKPOINT_ONLY" if passed else "HOLD_FOR_FIX",
            "passed": passed,
...
def validate(root: Path) -> list[Check]:
    ...
    checks.append(Check("hold:phase33", "HOLD_PHASE33_CHECKPOINT_ONLY" in combined, "Required Phase 33 hold marker."))
