"""Validate the Phase 15 control blueprint.

Phase 15 is a documentation and guard layer. It confirms that the repository
contains the blueprint markers while the runtime remains read-only plus planning.
"""
...
def main() -> int:
    parser = argparse.ArgumentParser(description="Validate the Phase 15 control blueprint.")
    parser.add_argument("--repo-root", default=".", help="Repository root. Defaults to current directory.")
    parser.add_argument(
        "--output",
        default="phase15-control-blueprint-validation-report.json",
        help="JSON report path.",
    )
    args = parser.parse_args()

    root = Path(args.repo_root).resolve()
    checks = validate_phase15_control_blueprint(root)
    passed = all(check.passed for check in checks)

    payload = {
        "summary": {
            "mode": "phase15_control_blueprint_validation",
            "phase15_control": "BLUEPRINT_ONLY" if passed else "CHECK_FAILED",
            "release_decision": "HOLD_PHASE15_CONTROL_BLUEPRINT_ONLY" if passed else "HOLD_FOR_FIX",
            "passed": passed,
...
def validate_phase15_control_blueprint(repo_root: Path) -> list[Check]:
    checks: list[Check] = []
    ...
    checks.append(_exists_check("phase15_validator_exists", repo_root / "scripts" / "phase15_control_blueprint_validate.py"))
    ...
    for marker in REQUIRED_PHASE15_MARKERS:
        checks.append(Check(f"phase15_marker:{marker}", marker in phase15_text, f"Phase 15 document must contain {marker}."))
