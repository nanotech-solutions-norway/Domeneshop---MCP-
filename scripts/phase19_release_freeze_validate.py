"""Validate the Phase 19 release freeze gate."""
...
def main() -> int:
    parser = argparse.ArgumentParser(description="Validate the Phase 19 release freeze gate.")
    parser.add_argument("--repo-root", default=".", help="Repository root. Defaults to current directory.")
    parser.add_argument("--output", default="phase19-release-freeze-validation-report.json", help="JSON report path.")
    args = parser.parse_args()

    root = Path(args.repo_root).resolve()
    checks = validate_phase19_release_freeze(root)
    passed = all(check.passed for check in checks)

    payload = {
        "summary": {
            "mode": "phase19_release_freeze_validation",
            "phase19_freeze": "FREEZE_ONLY" if passed else "CHECK_FAILED",
            "release_decision": "HOLD_PHASE19_RELEASE_FREEZE_ONLY" if passed else "HOLD_FOR_FIX",
            "passed": passed,
...
def validate_phase19_release_freeze(repo_root: Path) -> list[Check]:
    checks: list[Check] = []
    ...
    for marker in REQUIRED_HOLD_MARKERS:
        checks.append(Check(f"hold_marker:{marker}", marker in combined_text, f"Required marker must appear in release freeze text: {marker}."))
