"""Validate the Phase 36 write scope definition."""
...
def main() -> int:
    parser = argparse.ArgumentParser(description="Validate the Phase 36 write scope definition.")
    parser.add_argument("--repo-root", default=".")
    parser.add_argument("--output", default="phase36-write-scope-validation-report.json")
    ...
    payload = {
        "summary": {
            "mode": "phase36_write_scope_validation",
            "phase36_write_scope": "WRITE_SCOPE_DEFINITION_ONLY" if passed else "CHECK_FAILED",
            "release_decision": "HOLD_PHASE36_WRITE_SCOPE_DEFINITION_ONLY" if passed else "HOLD_FOR_FIX",
            "passed": passed,
...
def validate(root: Path) -> list[Check]:
    ...
    phase_doc = read(root / "docs" / DOCS[36])
    combined = "\n".join(read(root / "docs" / name) for name in DOCS.values()) + "\n" + read(root / "README.md")
    checks.append(Check("hold:phase36", "HOLD_PHASE36_WRITE_SCOPE_DEFINITION_ONLY" in combined, "Required Phase 36 hold marker."))
    ...
    for marker in REQUIRED_SCOPE_MARKERS:
        checks.append(Check(f"scope:{marker}", marker in phase_doc, f"Required scope marker: {marker}."))
    checks.append(Check("sequence:remaining", "Phase 37" in combined and "Phase 42" in combined, "Required remaining sequence markers."))
