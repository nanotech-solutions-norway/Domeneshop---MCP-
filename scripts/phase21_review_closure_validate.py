"""Validate the Phase 21 review closure gate."""
...
def main() -> int:
    parser = argparse.ArgumentParser(description="Validate the Phase 21 review closure gate.")
    parser.add_argument("--repo-root", default=".", help="Repository root. Defaults to current directory.")
    parser.add_argument("--output", default="phase21-review-closure-validation-report.json", help="JSON report path.")
    args = parser.parse_args()

    root = Path(args.repo_root).resolve()
    checks = validate_phase21_review_closure(root)
    passed = all(check.passed for check in checks)

    payload = {
        "summary": {
            "mode": "phase21_review_closure_validation",
            "phase21_closure": "CLOSURE_ONLY" if passed else "CHECK_FAILED",
            "release_decision": "HOLD_PHASE21_REVIEW_CLOSURE_ONLY" if passed else "HOLD_FOR_FIX",
            "passed": passed,
...
def validate_phase21_review_closure(repo_root: Path) -> list[Check]:
    checks: list[Check] = []
    ...
    for marker in REQUIRED_HOLD_MARKERS:
        checks.append(Check(f"hold_marker:{marker}", marker in combined_text, f"Required marker must appear in review closure text: {marker}."))
