"""Validate the Phase 22 maintenance baseline gate."""
...
def main() -> int:
    parser = argparse.ArgumentParser(description="Validate the Phase 22 maintenance baseline gate.")
    parser.add_argument("--repo-root", default=".", help="Repository root. Defaults to current directory.")
    parser.add_argument("--output", default="phase22-maintenance-baseline-validation-report.json", help="JSON report path.")
    args = parser.parse_args()

    root = Path(args.repo_root).resolve()
    checks = validate_phase22_maintenance_baseline(root)
    passed = all(check.passed for check in checks)

    payload = {
        "summary": {
            "mode": "phase22_maintenance_baseline_validation",
            "phase22_baseline": "BASELINE_ONLY" if passed else "CHECK_FAILED",
            "release_decision": "HOLD_PHASE22_MAINTENANCE_BASELINE_ONLY" if passed else "HOLD_FOR_FIX",
            "passed": passed,
...
def validate_phase22_maintenance_baseline(repo_root: Path) -> list[Check]:
    checks: list[Check] = []
    ...
    for marker in REQUIRED_HOLD_MARKERS:
        checks.append(Check(f"hold_marker:{marker}", marker in combined_text, f"Required marker must appear in maintenance baseline text: {marker}."))
