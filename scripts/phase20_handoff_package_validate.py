"""Validate the Phase 20 handoff package gate."""
...
def main() -> int:
    parser = argparse.ArgumentParser(description="Validate the Phase 20 handoff package gate.")
    parser.add_argument("--repo-root", default=".", help="Repository root. Defaults to current directory.")
    parser.add_argument("--output", default="phase20-handoff-package-validation-report.json", help="JSON report path.")
    args = parser.parse_args()

    root = Path(args.repo_root).resolve()
    checks = validate_phase20_handoff_package(root)
    passed = all(check.passed for check in checks)

    payload = {
        "summary": {
            "mode": "phase20_handoff_package_validation",
            "phase20_handoff": "HANDOFF_ONLY" if passed else "CHECK_FAILED",
            "release_decision": "HOLD_PHASE20_HANDOFF_PACKAGE_ONLY" if passed else "HOLD_FOR_FIX",
            "passed": passed,
...
def validate_phase20_handoff_package(repo_root: Path) -> list[Check]:
    checks: list[Check] = []
    ...
    for marker in REQUIRED_HOLD_MARKERS:
        checks.append(Check(f"hold_marker:{marker}", marker in combined_text, f"Required marker must appear in handoff package text: {marker}."))
